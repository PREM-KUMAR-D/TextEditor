...
void editorMoveCursor(int key)
{
    ...
    case ARROW_LEFT:
        ...
        break;
    case ARROW_RIGHT:
        if (E.cx != E.nScreenColumns - 1)
        {
            E.cx++;
        }
        break;
    case ARROW_DOWN:
        if (E.cy != E.nScreenRows - 1)
        {
            E.cy++;
        }
        break;
...
void editorProcessKeypress()
{
    ...
    case END_KEY:
        E.cx=E.nScreenColumns-1;
        break;
    case PAGE_UP:
    case PAGE_DOWN:
    {
        int times = E.nScreenRows;
        while(times--)
            editorMoveCursor(c==PAGE_DOWN?ARROW_UP:ARROW_DOWN);
    }
    break;
...
void editorDrawRows(struct abuf *ab)
{
    ...
    {
        if (y == E.nScreenRows / 3)
        {
            char welcome[80];
            int welcomeLen = snprintf(welcome, sizeof(welcome), "Kilo editor -- version %s", KILO_VERSION);
            if (welcomeLen > E.nScreenColumns)
                welcomeLen = E.nScreenColumns;
            int padding = (E.nScreenColumns - welcomeLen) / 2;
            if (padding)
            {
                abAppend(ab, "~", 1);
                padding--;
...
void initEditor()
{
    E.cx = 0;
    E.cy = 0;
    if (getWindowSize(&E.nScreenRows, &E.nScreenColumns) == -1)
        die("getWindowSize");
}
int main()
{
